...
def unwrap_in(rx):
    ...
    group_index = 1
    while '(!' in rx:
        rx = rx.replace('(!', '(?P<Select{}>'.format(group_index), 1)
        group_index += 1
    return rx
...
class SelectByRegexBase(sublime_plugin.TextCommand):
    ...
    def mark_groups(self, match):
        for group in match.groupdict():
            if re.match('Select\d*', group):
                self.inner_regions.append(sublime.Region(match.start(str(group)), match.end(str(group))))
